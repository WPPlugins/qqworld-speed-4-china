...
class qqworld_speed4china {
	...
	public function get_value() {
		...
		$this->using_custom_cdn_frontend = isset($this->values['using-custom-cdn-frontend']) ? $this->values['using-custom-cdn-frontend'] : 'disabled';
		$this->using_custom_cdn_frontend_from = isset($this->values['using-custom-cdn-frontend-from']) ? $this->values['using-custom-cdn-frontend-from'] : 'googleapis.com';
		$this->using_custom_cdn_frontend_to = isset($this->values['using-custom-cdn-frontend-to']) ? $this->values['using-custom-cdn-frontend-to'] : 'useso.com';
	...
	function cdn_callback($buffer) {
		if ($this->using_custom_cdn_frontend == 'enabled') return str_replace($this->using_custom_cdn_frontend_from, $this->using_custom_cdn_frontend_to, $buffer);
	...
	public function speed_up() {
		if (($this->using_custom_cdn_frontend == 'enabled' || $this->using_custom_cdn_frontend == 'delete') && !is_admin()) {
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				<tbody>
					<tr valign="top">
						<th scope="row">
							<label for="use-360-cdn"><?php _e('Using Custom Font lib on Front-End', 'qqworld-speed-4-china'); ?></label>
						</th>
						<td>
							<aside class="admin_box_unit">
								<label><input type="radio" id="use-custom-cdn-yes" name="qqworld-speed-4-china[using-custom-cdn-frontend]" value="enabled" <?php checked($this->using_custom_cdn_frontend, 'enabled'); ?> /> <?php _e('Enabled', 'qqworld-speed-4-china');_e('(Speed up)', 'qqworld-speed-4-china'); ?></label><br />
								<?php _e('Replace', 'qqworld-speed-4-china'); ?>
								<input type="text" name="qqworld-speed-4-china[using-custom-cdn-frontend-from]" placeholder="<?php _e('Such as: ', 'qqworld-speed-4-china'); ?>googleapis.com" value="<?php echo $this->using_custom_cdn_frontend_from; ?>" />
								<?php _e('To', 'qqworld-speed-4-china'); ?>
								<input type="text" name="qqworld-speed-4-china[using-custom-cdn-frontend-to]" placeholder="<?php _e('Such as: ', 'qqworld-speed-4-china'); ?>useso.com" value="<?php echo $this->using_custom_cdn_frontend_to; ?>" /><br />
								<label><input type="radio" id="use-custom-cdn-no" name="qqworld-speed-4-china[using-custom-cdn-frontend]" value="disabled" <?php checked($this->using_custom_cdn_frontend, 'disabled'); ?> /> <?php _e('Disabled', 'qqworld-speed-4-china'); ?></label><br />
								<label><input type="radio" id="use-custom-cdn-delete" name="qqworld-speed-4-china[using-custom-cdn-frontend]" value="delete" <?php checked($this->using_custom_cdn_frontend, 'delete'); ?> /> <?php _e('Delete Google Font', 'qqworld-speed-4-china'); ?></label>
							</aside>
						</td>
					</tr>
					<tr valign="top">
					...
					<tr valign="top">
						<th scope="row">
							<label for="auto-update-themes"><?php _e('Extend the Time of the Upgrade', 'qqworld-speed-4-china'); ?></label>
						</th>
						<td>
							<aside class="admin_box_unit">
								<label><input type="radio" id="extend-the-time-of-the-upgrade-yes" name="qqworld-speed-4-china[extend-the-time-of-the-upgrade]" value="enabled" <?php checked($this->extend_the_time_of_the_upgrade, 'enabled'); ?> /> <?php _e('Enabled', 'qqworld-speed-4-china'); ?></label><br />
								<label><input type="radio" id="extend-the-time-of-the-upgrade-no" name="qqworld-speed-4-china[extend-the-time-of-the-upgrade]" value="disabled" <?php checked($this->extend_the_time_of_the_upgrade, 'disabled'); ?> /> <?php _e('Disabled', 'qqworld-speed-4-china'); ?></label>
							</aside>
							<p class="description"><?php _e('If enabled this option, will never timeout when upgrading.', 'qqworld-speed-4-china'); ?></p>
						</td>
					</tr>
				</tbody>
			</table>
